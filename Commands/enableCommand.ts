...
async function Command(args: string[], message: Message, channelSettings: ChannelSettings) {
    ...
    if (!admin) {
        ...
        sendMessage(channel, "You must specify a command (truth, dare, etc.) or category (pg, irl, etc.) to enable.");
    }
    else {
        ...
        if (!settingNames.length) {
            ...
            let toBeEnabled: ValidChannelSetting[] = [];
            if (args.includes("all")) {
                    ...
                    if (categories.length !== 0) {
                        for (let category of categories) {
                            if (command === "dare" || (category !== "d" && category !== "irl")) {
                                toBeEnabled.push(<ValidChannelSetting>(command + " " + category));
                            }
                        }
                    }
                    else {
                        if (command !== "dare") {
                            ["pg", "pg13"].forEach(category => toBeEnabled.push(<ValidChannelSetting>(command + " " + category)));
                        }
                        else {
                            ["pg", "pg13", "d", "irl"].forEach(category => toBeEnabled.push(<ValidChannelSetting>("dare " + category)));
                        }
                    }
                }
            }
                ...
                for (let category of categories) {
                    if (category === "d" || category === "irl") {
                        toBeEnabled.push(<ValidChannelSetting>("dare " + category));
                    }
                    else {
                        ["truth", "dare", "wyr", "nhie", "paranoia"].forEach(command => toBeEnabled.push(<ValidChannelSetting>(command + " " + category)));
                    }
                }
            }

            if (toBeEnabled.length === 0) {
                sendMessage(channel, "Could not find any valid commands or categories to enable. Double check that any commands or categories specified are spelled correctly and are not mutually exclusive (like `truth` and `irl`).");
            }
            else {
...
async function SlashCommand(interaction: CommandInteraction, channelSettings: ChannelSettings) {
    ...
    for (let x of commandArray) {
        for (let y of categoryArray) {
            if (x === "dare" || (y !== "d" && y !== "irl")) {
                toBeEnabled.push(<ValidChannelSetting>(x + " " + y))
            }
        }
    }
