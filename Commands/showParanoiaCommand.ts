...
import { ChannelSetting, ChannelSettings, defaultSettings, handler, sendMessage } from '../bot.js';
...
async function SlashCommand(interaction: CommandInteraction, channelSettings: ChannelSettings) {
    ...
    if (server) {
        ...
        let cs = await handler.getChannelSettings(targetChannel!.id) ?? defaultSettings
